...
export default async function fetchAccountsByType(lastRun, accountTypeIds) {
  const token = await getAccessToken();


  const filter = await buildFilter(accountTypeIds);

  const url = `${ORG_URL}/api/data/v9.2/accounts?$select=name,accountnumber,accountid&$filter=modifiedon gt ${lastRun} and (${encodeURIComponent(filter)})`;
...
async function buildFilter(accountTypes) {
  return accountTypes.map(t => `statuscode eq 1 and new_membershipstatus eq ${t}`).join(' or ');
}
